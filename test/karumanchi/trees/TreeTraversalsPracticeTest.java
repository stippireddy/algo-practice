...
public class TreeTraversalsPracticeTest {
  @Test
  public void testTraversals1() {
    TreeNode root = TreeUtils.createTree(new Integer[] {null});
    Assert.assertEquals(TreeTraversalsPractice.preOrder(root), Arrays.asList(new Integer[] {}));
    Assert.assertEquals(TreeTraversalsPractice.inOrder(root), Arrays.asList(new Integer[] {}));
    Assert.assertEquals(TreeTraversalsPractice.postOrder(root), Arrays.asList(new Integer[] {}));
  }
  ...
  private void traversalTestHelper(Integer[] input, Integer[] preOrderExpected,
    ...
    List<Integer> result = TreeTraversalsPractice.preOrder(root);
    Assert.assertEquals(preOrderExpected, result.toArray(new Integer[result.size()]));
    result = TreeTraversalsPractice.preOrderRecursive(root);
    Assert.assertEquals(preOrderExpected, result.toArray(new Integer[result.size()]));

    result = TreeTraversalsPractice.inOrder(root);
    Assert.assertEquals(inOrderExpected, result.toArray(new Integer[result.size()]));
    result = TreeTraversalsPractice.inOrderRecursive(root);
    Assert.assertEquals(inOrderExpected, result.toArray(new Integer[result.size()]));

    result = TreeTraversalsPractice.postOrder(root);
    Assert.assertEquals(postOrderExpected, result.toArray(new Integer[result.size()]));
    result = TreeTraversalsPractice.postOrderRecursive(root);
    Assert.assertEquals(postOrderExpected, result.toArray(new Integer[result.size()]));
  }
}
